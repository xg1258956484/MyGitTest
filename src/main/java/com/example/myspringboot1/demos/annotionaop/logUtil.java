 ...
 * @author xieGang
 * @description
 * @date 2024/5/12 下午7:21
 **/
@Component
@Aspect
public class logUtil {

    @Pointcut("execution(public Integer com.example.myspringboot1.demos.annotionaop.MyCalculator.*  (..))")
    ...
    public static void stop(JoinPoint joinPoint,Object result){
        Signature signature = joinPoint.getSignature();
        System.out.println("logAfterReturning通知---"+signature.getName()+"方法执行结束，结果是："+result);
    }

    @AfterThrowing(value = "myPointCut()",throwing = "e")
    public static void logException(JoinPoint joinPoint,Exception e){
        Signature signature = joinPoint.getSignature();
        System.out.println("logAfterThrowing通知---"+signature.getName()+"方法抛出异常："+e.getMessage());
    }
}
